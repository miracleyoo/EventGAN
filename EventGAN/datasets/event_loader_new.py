...
class MVSECSequence(Dataset):
    ...
    def load(self, only_length=False):
        ...
        if not only_length:
            # print(self.sequence['events'][0].dtype)
            self.events = np.concatenate(self.sequence['events'])
            self.events = np.stack([ self.events['x'], self.events['y'], self.events['timestamp'], self.events['polarity']], axis=1)
            # print(self.events.shape)
        self.images_ts = self.sequence['timestamps']
        self.image_to_event = [len(self.sequence['events'][i]) for i in range(len(self.sequence['events']))]
        for i in range(1,len(self.image_to_event)):
            self.image_to_event[i] += self.image_to_event[i-1]
